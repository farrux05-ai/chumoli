"""
chumoli.security.api_key
========================

Shared-secret API key for /api/* routes (except /api/health).

Key resolution order:
1. CHUMOLI_API_KEY environment variable
2. ~/.chumoli/api.key (or $CHUMOLI_HOME/api.key), mode 600
...
def _default_key_path() -> Path:
    home = Path(os.environ.get("CHUMOLI_HOME") or (Path.home() / ".chumoli"))
...
def load_or_create_api_key(key_path: Path | None = None) -> str:
    ...
    env = os.environ.get("CHUMOLI_API_KEY", "").strip()
    if env:
        return env
    ...
    path.chmod(0o600)
    print(
        f"[chumoli] API key yaratildi va saqlandi: {path}\n"
        f"[chumoli] CHUMOLI_API_KEY={value}\n"
        f"[chumoli] Dashboard so'rovlari uchun X-API-Key header kerak.",
        flush=True,
    )
    return value
